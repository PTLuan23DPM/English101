...
# Transcripts for remaining files
transcripts = {
...
I'm John Russell.""",
    ...
    # Advanced - American Stories (from summary)
    "_The Fall of the House of Usher,_ by Edgar Allan Poe, Part One.json": """Our story today is called "The Fall of the House of Usher," by Edgar Allan Poe. The story was originally adapted and recorded by the U.S. Department of State.
...
"I shall die," he said. "I shall die! I must die of this fool's sickness. In this way, this way and no other way, I shall be lost. I fear what will happen in the future, not for what happens, but for the result of what happens. I have, indeed, no fear of pain, but only fear of its result - of terror! I feel that my time will soon arrive when I must lose my life, and my mind, and my soul, together, in some last battle with that horrible enemy: fear!""",
    ...
    "_The Fall of the House of Usher,_ by Edgar Allan Poe, Part Two.json": """We present the second of three parts of the short story "The Fall of the House of Usher," by Edgar Allan Poe. The story was originally adapted and recorded by the U.S. Department of State.
...
The words of one of his songs, called "The Haunted Palace," I have easily remembered. In it I thought I saw, and for the first time, that Usher knew very well that his mind was weakening. This song told of a great house where a king lived — a palace — in a green valley, where all was light and color and beauty, and the air is sweet. In the palace were two bright windows through which people in that happy valley could hear music and could see smiling ghosts — spirits — moving around the king. The palace door was of the richest materials, in red and white; through it came other spirits whose only duty was to sing in their beautiful voices about how wise their king was.
...
Now it's your turn to use the words in this story. How do you act in scary situations? How do you in act in very sad situations? Let us know in the comments section or on our Facebook page.""",
    ...
    "_The Open Boat_ by Stephen Crane, Part One.json": """Our story today is called "The Open Boat." It was written by Stephen Crane and is based on what really happened to him in eighteen ninety-six.
...
def main():
    ...
    # File mappings: (relative_path, transcript_key)
    files_to_update = [
        # Intermediate
        (os.path.join("Intermediate", "Arts & Culture", "The Westminster Kennel Club Dog Show, the 'Super Bowl' of Dog Shows.json"), "The Westminster Kennel Club Dog Show, the 'Super Bowl' of Dog Shows.json"),
        (os.path.join("Intermediate", "As It Is", "Researchers South Korea's Birth Rate Increase Last Year Unclear.json"), "Researchers South Korea's Birth Rate Increase Last Year Unclear.json"),
        # Advanced
        (os.path.join("Advanced", "American Stories", "_The Fall of the House of Usher,_ by Edgar Allan Poe, Part One.json"), "_The Fall of the House of Usher,_ by Edgar Allan Poe, Part One.json"),
        (os.path.join("Advanced", "American Stories", "_The Fall of the House of Usher,_ by Edgar Allan Poe, Part Two.json"), "_The Fall of the House of Usher,_ by Edgar Allan Poe, Part Two.json"),
        (os.path.join("Advanced", "American Stories", "_The Fall of the House of Usher,_ by Edgar Allan Poe, Part Three.json"), "_The Fall of the House of Usher,_ by Edgar Allan Poe, Part Three.json"),
        (os.path.join("Advanced", "American Stories", "_The Open Boat_ by Stephen Crane, Part One.json"), "_The Open Boat_ by Stephen Crane, Part One.json"),
    ]
    ...
    for rel_path, transcript_key in files_to_update:
        filepath = os.path.join(base_dir, rel_path)
        if transcript_key not in transcripts:
            print(f"  [NO TRANSCRIPT] {os.path.basename(filepath)}")
            continue
        ...
        success, status = update_json_file(filepath, transcripts[transcript_key])
        if success:
