...
export default function SpeakingTaskManager() {
  const [tasks, setTasks] = useState<SpeakingTask[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [showForm, setShowForm] = useState(false);
  const [editingTask, setEditingTask] = useState<SpeakingTask | null>(null);
  const [formData, setFormData] = useState({
    ...
    timeLimit: "",
    tips: [] as string[],
    vocab: [] as Array<{ word: string; ipa: string }>,
    phrases: [] as string[],
    recommended: false,
    icon: "🎤",
    color: "blue",
  ...
  const fetchTasks = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await fetch("/api/admin-dashboard/courses/speaking-tasks");
      ...
      setError(msg);
      console.error("[SpeakingTaskManager] Error:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddVocab = () => {
    if (vocabWord.trim() && vocabIpa.trim()) {
      setFormData({
        ...formData,
        vocab: [...formData.vocab, { word: vocabWord.trim(), ipa: vocabIpa.trim() }],
      });
      setVocabWord("");
      setVocabIpa("");
    }
  ...
  const handleRemoveVocab = (index: number) => {
    setFormData({
      ...formData,
      vocab: formData.vocab.filter((_, i) => i !== index),
    });
  };

  const handleOpenForm = (task?: SpeakingTask) => {
    if (task) {
      setEditingTask(task);
      setFormData({
        title: task.title,
        ...
        tips: task.tips || [],
        vocab: Array.isArray(task.vocab) ? task.vocab : [],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingTask
        ? `/api/admin-dashboard/courses/speaking-tasks/${editingTask.id}`
        : "/api/admin-dashboard/courses/speaking-tasks";
      ...
      const msg = err instanceof Error ? err.message : "Unknown error";
      toast.error(msg);
      console.error("[SpeakingTaskManager] Submit error:", err);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/admin-dashboard/courses/speaking-tasks/${id}`, {
        method: "DELETE",
      });
      ...
      const msg = err instanceof Error ? err.message : "Unknown error";
      toast.error(msg);
      console.error("[SpeakingTaskManager] Delete error:", err);
    }
  };
            ...
            <div key={task.id} className="activity-card">
              <div className="activity-header">
                ...
                <div className="activity-badges">
                  ...
                  <span className="skill-badge">SPEAKING</span>
              ...
              <div className="activity-meta">
                <span>Time Limit: {task.timeLimit}</span>
                <span>Tips: {task.tips?.length || 0}</span>
                <span>Vocab: {Array.isArray(task.vocab) ? task.vocab.length : 0}</span>
                <span>Phrases: {task.phrases?.length || 0}</span>
              </div>
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g., Pronunciation, Topic Discussion"
                      required
                    />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <div style={{ display: "flex", gap: "8px", marginBottom: "8px" }}>
                    <input
                      ...
                      placeholder="Enter a phrase and press Enter"
                      style={{ flex: 1 }}
                    />
                    <button
                      type="button"
                ...
                <div>
                  ...
                  {formData.vocab.length > 0 && (
                    <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                      {formData.vocab.map((v, idx) => (
                        <div
                          key={idx}
                          className="tag"
                          style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "8px" }}
                        >
                          <span>{v.word} - {v.ipa}</span>
                          <button
                            type="button"
